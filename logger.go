...
func appendAttrs(pairs []Pair, p *localeutil.Printer, attrs map[string]any) []Pair {
	if len(attrs) == 0 {
		panic("参数 attrs 不能为空")
	}

	for k := range attrs {
		if sliceutil.Exists(pairs, func(v Pair, _ int) bool { return v.K == k }) {
			panic(fmt.Sprintf("存在同名的元素 %s", k))
		}
	}

	return append(pairs, attrs2Pairs(p, attrs)...)
}
...
func (l *Logger) AppendAttrs(attrs map[string]any) {
	l.pairs = appendAttrs(l.pairs, l.logs.printer, attrs)
}
...
// 转换成 io.Writer
//
// 仅供 [Logger.StdLogger] 使用，因为 depth 值的关系，只有固定的调用层级关系才能正常显示行号。
